...
class HDFCOpage extends StatefulWidget {
  const HDFCOpage({super.key});

  @override
  State<HDFCOpage> createState() => _HDFCOpageState();
}

class _HDFCOpageState extends State<HDFCOpage> {
  final HDFCOController hdfcController = Get.find();
  ...
  void initState() {
    ...
    _getaxisDocument().then((snapshot) {
      ...
      Timer timer = Timer.periodic(Duration(seconds: 2), (timer) {
        percentChange(hdfcController.hdfcoPrice, _prevClose);
      });
    });
  }
